...
def peliculas_duracion(Pelicula: str):
    ...
    return {'Pelicula': Pelicula, 'Duración': duracion, 'Año de lanzamiento': año}
...
def franquicia(Franquicia: str):
    ...
    if franquicia.empty:
        raise HTTPException(status_code=404, detail="No se encontró la franquicia en el dataset")

    cantidad_peliculas = franquicia.shape[0]
    ganancia_total = franquicia['revenue'].sum()
    ganancia_promedio = franquicia['revenue'].mean()

    return {'Franquicia': Franquicia, 'Cantidad de películas': cantidad_peliculas, 'Ganancia total': ganancia_total, 'Ganancia promedio': ganancia_promedio}
...
def peliculas_pais(Pais: str):
    ...
    return {'País': Pais, 'Cantidad de películas': cantidad_peliculas}
...
def productoras_exitosas(Productora: str):
    ...
    return {'Productora': Productora, 'Revenue total': revenue_total, 'Cantidad de películas': cantidad_peliculas}
...
def get_director(nombre_director: str):
    ...
    if director_films.empty:
        raise HTTPException(status_code=404, detail="No se encontró información para el director: {}".format(nombre_director))
    ...
    return {'Director': nombre_director, 'Retorno total del director': exito_director}, detalles_peliculas
